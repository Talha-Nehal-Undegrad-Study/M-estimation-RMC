...
for i = 1:maxiter
    ...
    for j = 1: n2
        [vj_kupd, scale_kupd, iter, ~] = hubreg(getxj(X_Omega, indices_col, j), Uk(over non-zero rows), c, sigma_cell_for_v{j}, v_upd(:, j), n2);
        v_upd(:, j) = vj_kupd;
    ...
    u_upd = u_cell{i};
    for z = 1:n1
       [ui_upd, scale_kupd, iter, ~] = hubreg(getxi(X_Omega, indices_row, z), Vk(over non-zero columns), c, sigma_for_u{z}, u_upd(z, :), n1);
        u_upd(z, :) = ui_upd;
