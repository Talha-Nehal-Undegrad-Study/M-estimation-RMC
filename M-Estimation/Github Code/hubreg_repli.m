...
for i = 1:maxiter
    ...
    for j = 1: n2
        [vj_kupd, scale_kupd, ~] = real_hubreg(getxj(X_Omega, indices_col, j), get_u(u_cell{i}, indices_col, j), c);
    ...
    u_upd = u_cell{i};
    for z = 1:n1
       [ui_upd, scale_kupd, ~] = real_hubreg(getxi(X_Omega, indices_row, z), (get_v(v_cell{i + 1}, indices_row, z))', c);
