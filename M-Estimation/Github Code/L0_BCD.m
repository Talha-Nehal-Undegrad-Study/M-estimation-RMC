function [Out_X,MSE,loss] = L0_BCD(M, M_Omega, Omega_array, rak, maxiter)
%%
...
for iter = 1 : maxiter
    ...
    M_Omega_update = (M_Omega - S).*Omega_array;
...
w = exp(-(abs(input_data-mean(input_data))./(A)));%P��ʾȨ��
y = find(w<1e-20);  
end